...
public:
    ...
    void insert(key_type key) {
        //Creates new root with key if no root exists
        if (root_ == nullptr) {
            root_ = new node_type(key, 0);
            root_->parent_ = nullptr;
            size_++;
        }
        else {
            auto target_node = root_;
            //Breaks once an empty node is found, or a duplicant key is found.
            while (target_node != nullptr && target_node->key_ != key) {
                //This finds the direction that the node goes in relation to its parent, avoiding an if else branch.
                bool dir = key > target_node->key_;
                if (target_node->children_[dir] == nullptr) {
                    target_node->children_[dir] = new node_type(key, 1, nullptr, nullptr, target_node);
                    size_++;
                    //Insert check is the function that maintains the red black tree invariant, so see that function for more details.
...
public:
     ...
     */
    void generate_dot(std::string out_fpath) {
        std::string text;
        if (!root_){
...
public:
    ...
    some_key_type min() {
        auto min_node = root_;
        auto target_node = root_;
        //Finds the leftmost node, which is the min of the tree.
        ...
        if (min_node){
            return some_key_type(min_node->key_);
        }
...
public:
     ...
     */
    static RedBlackTree<key_type>* merge(RedBlackTree<key_type>* tree1, RedBlackTree<key_type>* tree2){
     ...
     */
    std::pair<RedBlackTree<key_type>*,RedBlackTree<key_type>*>  split(key_type pivot) {
        ...
        root_ = nullptr;
        size_ = 0;
        return std::pair<RedBlackTree<key_type>*,RedBlackTree<key_type>*>(tree1, tree2); 
    };
...
private:
    ...
    void insert_check(Node<Key_>* node) {
        ...
        //Cases 2-4:...
            ...
            //Case 2: Perform a rotation dependant on the node's direction and the parent's direction relative to their parents, and
            //Swap the color of the node and its children.
            if (black_sibling) {
     ...
     * @return std::string 
     */
    std::string write_dot(node_ptr node, std::string text = "") {
        if (node){
